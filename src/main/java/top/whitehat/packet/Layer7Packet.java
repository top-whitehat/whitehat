...
/**
 * A base class for packets of OSI Layer 7 protocols that is based on TCP or UDP.​
 */
public abstract class Layer7Packet extends Packet implements IHasMac, IHasIpPort {
	...
	public Layer7Packet srcIp(InetAddress addr) {
		if (_parent instanceof IHasIp) {
	...
	public Layer7Packet dstIp(InetAddress addr) {
		if (_parent instanceof IHasIp) {
	...
	public Layer7Packet srcMac(MacAddress addr) {	
		if (_parent instanceof IHasMac) {
	...
	public Layer7Packet dstMac(MacAddress addr) {
		if (_parent instanceof IHasMac) {
			((IHasMac)_parent).srcMac(addr);
		}
		return this;
	...
	/** set source port */
	public Layer7Packet srcPort(int value) {
		if (_parent instanceof IHasIpPort) {
	...
	public Layer7Packet dstPort(int value) {
		if (_parent instanceof IHasIpPort) {
