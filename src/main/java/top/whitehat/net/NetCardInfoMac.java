...
/**
 * Read network information in Mac OS, such as gateway IP and MAC address.
 */
public class NetCardInfoMac {
	...
	public static String getGatewayIP() {
		String str = CommandLine.run("route -n get default").getOutput();
		String ret = Text.of(str).grep("gateway").split(":").cell(0, 1);
	...
	public static String getMACAddress(String ipAddress) {
		...
		Text t = Text.of(str).grep("at").split(" \t");
		if (t.rows() > 0) {
			return t.table.row(0).between("at", "on").value(0);
		}
		return null;
	}
